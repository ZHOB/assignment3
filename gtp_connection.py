...
class GtpConnection:
    ...
    def policy_moves(self,args):
        move_list = self.board.Win()
        final_list = []
        if move_list != []:
            for move in move_list:
                coords = point_to_coord(move, self.board.size)
                final_list.append(format_point(coords))
            sorted_moves = " ".join(sorted(final_list))
            self.respond("Win " + sorted_moves)
            return
        move_list = self.board.BlockWin()
        if move_list != []:
            for move in move_list:
                coords = point_to_coord(move, self.board.size)
                final_list.append(format_point(coords))
            sorted_moves = " ".join(sorted(final_list))
            self.respond("BlockWin " + sorted_moves)
            return
        move_list = self.board.OpenFour()
        if move_list != []:
            for move in move_list:
                coords = point_to_coord(move, self.board.size)
                final_list.append(format_point(coords))
            sorted_moves = " ".join(sorted(final_list))
            self.respond("OpenFour " + sorted_moves)
            return
        move_list = self.board.BlockOpenFour()
        if move_list != []:
            for move in move_list:
                coords = point_to_coord(move, self.board.size)
                final_list.append(format_point(coords))
            sorted_moves = " ".join(sorted(final_list))
            self.respond("BlockOpenFour " + sorted_moves)
            return
        move_list = self.board.get_empty_points().tolist()
        if move_list == []:
            self.respond()
            return
        for move in move_list:
    ...
    def genmove_cmd(self, args):
        ...
        board_color = args[0].lower()
        color = color_to_int(board_color)
        move = self.go_engine.simulation(self.board, color,"rule_based")
